/**
 * Handles modal submission for beast file upload
...
module.exports = async (interaction, _client) => {
  ...
  const fileField = interaction.fields.fields.get('beast_file');
  ...
  if (
    ...
    const description = await generateImageDescription(data.imageUrls[0], 'beast');
    if (description) {
  ...
  try {
    ...
    if (normalizedData.imageUrls && normalizedData.imageUrls.length > 0) {
      ...
      const submitRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`imageBeastAdditionNo_${normalizedData.name}`)
      ...
      const actionRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId('imageBeastAdditionYes')
          .setLabel('Add Images')
          .setStyle(ButtonStyle.Primary)
          .setEmoji('🖼️'),
        new ButtonBuilder()
          .setCustomId('imageBeastAdditionNo')
