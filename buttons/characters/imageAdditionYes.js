...
module.exports = async (interaction, _client) => {
  ...
  if (!role) {
    await interaction.editReply({
      content:
        'Role not found. Please ensure the role exists and the bot has the appropriate permissions.',
      flags: [64],
    });
    return;
  }
  ...
  try {
    ...
    await interaction.update({
      content:
        "You've been assigned the 'Character Image Upload' role. Please upload your images now.",
      components: [],
      flags: [64],
    });
    ...
    collector.on('collect', async (m) => {
      ...
      try {
        await postCharacterInfo(interaction, _client, characterName, imageUrls);
        await interaction.deleteReply();
        await interaction.followUp({
          content: 'Character information and images have been submitted for staff approval.',
          flags: [64],
        });
      } catch (error) {
        console.error('Failed to post character information:', error);
        await interaction.followUp({
          content: 'There was an error processing your character information request.',
          flags: [64],
        });
      }
