...
async function handleDeleteCharacterInteraction(interaction) {
  ...
  const confirmButton = new ButtonBuilder()
    .setCustomId(`confirmDeleteCharacter_${characterId}_${userId}`)
    .setLabel('Yes, Delete')
  ...
  await interaction.reply({
    content: `⚠️ **Are you sure you want to delete the character "${characterId}"?**\n\nThis will archive the character and remove it from the list. This action cannot be easily undone.`,
    components: [row],
    flags: [64],
  });
}

async function handleConfirmDeleteCharacter(interaction) {
  const db = getDb();
...
module.exports = { handleDeleteCharacterInteraction, handleConfirmDeleteCharacter };
