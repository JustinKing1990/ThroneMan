...
async function handleDeleteLoreInteraction(interaction) {
    ...
    const targetChannel = await interaction.client.channels.fetch('1207398646035910726').catch(console.error);

    const [action, loreId] = interaction.customId.split('_');
                ...
                if (deletionResult.deletedCount === 0) {
                    ...
                    messages.forEach(async message => {
                        if (message.author.bot && message.embeds.length > 0) {
                            ...
                            const hasLoreName = embed.fields && embed.fields.some(field => field.value.includes(loreId));
                            if (hasLoreName) {
