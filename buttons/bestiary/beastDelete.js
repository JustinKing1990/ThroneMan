...
async function handleDeleteLoreInteraction(interaction) {
    ...
    try {
        let newBeastCollection = db.collection('lore');
        await updateListMessage(null, interaction, newBeastCollection, settingsCollection, config.bestiaryChannelId, config.bestiaryMessageId, "Beast");
    } catch (error) {
