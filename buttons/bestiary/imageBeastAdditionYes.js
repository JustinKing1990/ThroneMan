...
module.exports = async (interaction, client) => {
  ...
  let targetChannel = await interaction.client.channels.fetch(
    "1209676283794034728"
  );
  ...
  try {
      ...
      collector.on("end", async (collected) => {
        if (collected.size === 0) {
          ...
          await interaction.followUp({
            content:
              "No images were uploaded in time. Click the button below to try again.",
    ...
    await interaction.update({
      content:
        "There was an error processing the beast approval. Yell at your local dev",
      ephemeral: true,
    });
  }
};
